...
function boritolink($idbook) {
	...
	$res=$db->query("select * from books where idbook=$idbook")
		or die ("Nem sikerült a lekérdezés! Az sql a következő volt: <i>$sql</i>");
	$r=$res->fetch();

	if ($r['coverfile']) {$cover="http://".$_SERVER['SERVER_NAME']."/docs/webcovers/".$r['coverfile'];}
		else {$cover="http://".$_SERVER['SERVER_NAME']."/docs/no_picture.png";}
	$id=$r['idbook'];
	$title=$r['title'];
	...
	$cedula="http://".$_SERVER['SERVER_NAME']."/cedula/?id=$id";

	print "<a href='$cedula' target='_blank'><img class='alignnone' style='height: 200px;' title='$fulltitle $abstractshort' src='$cover' alt='$fulltitle' /></a>";
}
